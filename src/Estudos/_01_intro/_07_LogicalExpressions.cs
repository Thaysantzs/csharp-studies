...
class LogicalExpressions
{
    public static void Main(string[] args)
    {
        ...
        Console.WriteLine($"Salary greater than 150.000 and is manager = {salary > 150000 && isManager}");
        Console.WriteLine($"is married, salary greater than 150.000 and is manager = {isMarried && salary > 150000 && isManager}");
        Console.WriteLine(
        $"Not married and salary greater than or equal to 150000 or is not manager = {((!isMarried) && salary >= 15000) || (!isManager)}"
        );
    }
}